"""
build/platform/linux.py — NextSSL Linux native build helper.
...
def build(variant: str, build_dir: Path, bin_dir: Path, jobs: int,
    ...
    logger = _make_logger("nextssl.build.linux", level, log_path)

    logger.info("NextSSL Linux Build  variant=%s", variant)
    ...
    toolchain = Path(__file__).parent / "linux.cmake"
    cmake_flags = [
        cmake, str(ROOT),
    ...
    artifacts = list(bin_dir.glob("*.so")) + list(bin_dir.glob("*.a"))
    if artifacts:
