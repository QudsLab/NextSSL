...
# Export list for the primary/system binary (full variant).
...
def build(builder: Builder):
    """Build system.dll — full primary binary."""
    ...
    add_sources([os.path.join(src_dir, 'common/encoding/')], recursive=False)  # all radix encoders
    ...
    root_sub_files = [
        ...
        'interfaces/root/legacy/root_legacy.c',
        'interfaces/root/radix/root_radix.c',
    ...
    # Layer 3 (main/full) dispatch implementations
    add_sources([os.path.join(src_dir, 'interfaces/main/full/')], recursive=False)
    # Layer 2 (core) implementations
    add_sources([os.path.join(src_dir, 'interfaces/core/')], recursive=True)
    # Layer 2 core base implementations in excluded subdirectories
    _core_base_files = [
    ...
    # Filter: PQCrypto/common/drbg conflicts with seed/drbg (same symbols)
