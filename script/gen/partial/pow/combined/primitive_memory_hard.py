...
def build(builder: Builder):
    """Build PoW combined DLL (server+client) for primitive memory hard algorithms."""
    ...
    # Wrapper
    wrapper = os.path.join(src_dir, 'utils/pow/combined/primitive_memory_hard.c')
    ...
    return builder.build_target(
        ...
        macros=['POW_ENABLE_SERVER', 'POW_ENABLE_CLIENT', 'POW_ENABLE_PRIMITIVE_MEMORY_HARD']
    )

if __name__ == "__main__":
    from script.core import Config, Logger
    config = Config()
    with Logger(config.get_log_path('partial/pow/combined', 'primitive_memory_hard')) as logger:
