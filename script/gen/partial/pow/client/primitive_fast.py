...
def build(builder: Builder):
    """Build PoW client DLL for primitive fast algorithms."""
    src_dir = builder.config.src_dir
    
    # Core PoW logic + Client logic + Adapter implementation
    ...
    # Wrapper
    wrapper = os.path.join(src_dir, 'utils/pow/client/primitive_fast.c')
    ...
    return builder.build_target(
        ...
        macros=['POW_ENABLE_CLIENT', 'POW_ENABLE_PRIMITIVE_FAST']
    )

if __name__ == "__main__":
    from script.core import Config, Logger
    config = Config()
    with Logger(config.get_log_path('partial/pow/client', 'primitive_fast')) as logger:
