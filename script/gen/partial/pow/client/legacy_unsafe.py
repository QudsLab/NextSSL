...
def build(builder: Builder):
    """Build PoW client DLL for legacy unsafe algorithms."""
    ...
    # Wrapper
    wrapper = os.path.join(src_dir, 'utils/pow/client/legacy_unsafe.c')
    ...
    return builder.build_target(
        ...
        macros=['POW_ENABLE_CLIENT', 'POW_ENABLE_LEGACY_UNSAFE']
    )

if __name__ == "__main__":
    from script.core import Config, Logger
    config = Config()
    with Logger(config.get_log_path('partial/pow/client', 'legacy_unsafe')) as logger:
