...
def build(builder: Builder):
    """Build PoW client DLL for primitive sponge xof algorithms (SHA3)."""
    ...
    # Wrapper
    wrapper = os.path.join(src_dir, 'utils/pow/client/primitive_sponge_xof.c')
    ...
    return builder.build_target(
        ...
        macros=['POW_ENABLE_CLIENT', 'POW_ENABLE_PRIMITIVE_SPONGE_XOF']
    )

if __name__ == "__main__":
    from script.core import Config, Logger
    config = Config()
    with Logger(config.get_log_path('partial/pow/client', 'primitive_sponge_xof')) as logger:
