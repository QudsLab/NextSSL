...
def build(builder: Builder):
    """Build PoW server DLL for legacy unsafe algorithms."""
    ...
    # Wrapper
    wrapper = os.path.join(src_dir, 'utils/pow/server/legacy_unsafe.c')
    ...
    return builder.build_target(
        ...
        macros=['POW_ENABLE_SERVER', 'POW_ENABLE_LEGACY_UNSAFE']
    )

if __name__ == "__main__":
    from script.core import Config, Logger
    config = Config()
    with Logger(config.get_log_path('partial/pow/server', 'legacy_unsafe')) as logger:
