...
def build(builder: Builder):
    """Build primitive_sponge_xof.dll."""
    src_dir = builder.config.src_dir
    
    # src/primitives/hash/sponge_xof/
    sources = builder.get_sources([
        os.path.join(src_dir, 'primitives', 'hash', 'sponge_xof')
    ], recursive=True)
    
    wrapper = os.path.join(src_dir, 'utils', 'hash', 'primitive_sponge_xof.c')
...
if __name__ == "__main__":
    config = Config()
    with Logger(config.get_log_path('partial/hash', 'primitive_sponge_xof')) as logger:
