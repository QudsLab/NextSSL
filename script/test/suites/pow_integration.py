...
def load_dll_pair(category):
    ...
    server_path = os.path.join(bin_dir, f'server/{category}.dll')
    client_path = os.path.join(bin_dir, f'client/{category}.dll')
...
def main():
    ...
    categories = {
        'primitive_fast': ['sha256', 'blake3'],
        'primitive_memory_hard': ['argon2id', 'argon2i', 'argon2d'],
        'primitive_sponge_xof': ['shake128', 'shake256'],
    ...
    for category, algos in categories.items():
        ...
        if server and client:
            if not boss_control_loop(server, client, algos, difficulty=1):
