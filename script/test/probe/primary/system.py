"""script/test/probe/primary/system.py — structural probe for primary/system (full).

Checks every symbol in _WASM_SYSTEM_EXPORTS exists and is type-assignable.
...
from script.gen.primary.system    import _WASM_SYSTEM_EXPORTS
...
def main() -> int:
    config   = Config()
    dll_path = config.get_lib_path('primary', 'main')
    ...
    r = Results('probe/primary/system')
    for sym in _WASM_SYSTEM_EXPORTS:
        if sym in _SKIP:
            continue
