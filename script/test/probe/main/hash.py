"""script/test/probe/main/hash.py — structural probe for hash.dll.

Checks every symbol in _WASM_HASH_EXPORTS exists and is type-assignable.
...
from script.gen.main.hash    import _WASM_HASH_EXPORTS
...
def main() -> int:
    ...
    r = Results('probe/main/hash')
    for sym in _WASM_HASH_EXPORTS:
        if sym in _SKIP:
            continue
