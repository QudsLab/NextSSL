...
def main():
    """Test server + client + combined DLLs for legacy_unsafe algorithms."""
    
    config = Config()
    server_dll_path = config.get_lib_path('partial', 'legacy_unsafe', 'pow', 'server')
    client_dll_path = config.get_lib_path('partial', 'legacy_unsafe', 'pow', 'client')
    ...
    # --- Test Logic ---
    console.print_header("Starting Legacy Unsafe PoW Test")
    ...
    for algo in algos:
        for difficulty in difficulties:
            ...
            console.log_only(f"  Target (Bin): {bytes_to_bin(target_bytes)}")
            console.log_only(f"  Target Prefix Bits: {prefix_bits(target_bytes, challenge.difficulty_bits)}")
            ...
            console.log_only(f"  Hash (Bin): {bytes_to_bin(hash_bytes)}")
            console.log_only(f"  Hash Prefix Bits: {prefix_bits(hash_bytes, challenge.difficulty_bits)}")
