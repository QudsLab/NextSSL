...
def main():
    """Test server + client + combined DLLs for legacy_alive algorithms."""
    
    config = Config()
    server_dll_path = config.get_lib_path('partial', 'legacy_alive', 'pow', 'server')
    client_dll_path = config.get_lib_path('partial', 'legacy_alive', 'pow', 'client')
    ...
    # --- Test Logic ---
    console.print_header("Starting Legacy Alive PoW Test")
