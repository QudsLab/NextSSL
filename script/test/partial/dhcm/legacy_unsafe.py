...
def main():
    try:
        ...
        DLL_PATH = os.path.join(PROJECT_ROOT, 'bin', 'partial', 'dhcm', 'legacy_unsafe.dll')
        ...
        # MD2 Test
        # ---------------------------------------------------------
        console.print_step("Testing MD2")
        params = DHCMParams(
            algorithm=0x0500, # DHCM_MD2
            input_size=16,
            output_size=16
        )
        res = DHCMResult()
        ...
        if lib.leyline_dhcm_calculate(ctypes.byref(params), ctypes.byref(res)) == 0:
            if res.work_units_per_eval == 2400:
                console.print_pass("MD2 (16B)")
                print(f"       WU: {res.work_units_per_eval}")
                passed += 1
            else:
                console.print_fail(f"MD2 mismatch. Expected 2400, got {res.work_units_per_eval}")
                failed += 1
        else:
            console.print_fail("MD2 calculation failed")
            failed += 1

        # ---------------------------------------------------------
        # MD4 Test
        # ---------------------------------------------------------
        console.print_step("Testing MD4")
        params.algorithm = 0x0501 # DHCM_MD4
        params.input_size = 32
        
        if lib.leyline_dhcm_calculate(ctypes.byref(params), ctypes.byref(res)) == 0:
            if res.work_units_per_eval == 400:
                console.print_pass("MD4 (32B)")
                print(f"       WU: {res.work_units_per_eval}")
                passed += 1
            else:
                console.print_fail(f"MD4 mismatch. Expected 400, got {res.work_units_per_eval}")
                failed += 1
        else:
            console.print_fail("MD4 calculation failed")
            failed += 1

        print(f"\n{'='*50}")
        if failed == 0:
            console.print_pass(f"DHCM Legacy Unsafe: {passed} passed")
            return 0
        else:
            console.print_fail(f"DHCM Legacy Unsafe: {failed} failed")
            return 1
