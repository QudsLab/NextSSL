...
def main():
    try:
        ...
        DLL_PATH = os.path.join(PROJECT_ROOT, 'bin', 'base', 'dhcm_legacy.dll')
        ...
        if lib.leyline_dhcm_calculate(ctypes.byref(params), ctypes.byref(res)) == 0 and res.work_units_per_eval == 2400:
            ...
            console.print_fail(f"MD2 Failed. Expected 2400, got {res.work_units_per_eval}")
            failed += 1

        print(f"\n{'='*50}")
        if failed == 0:
            console.print_pass(f"DHCM Base Legacy: {passed} passed")
            return 0
        else:
            console.print_fail(f"DHCM Base Legacy: {failed} failed")
            return 1
