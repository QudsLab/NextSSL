...
int nextssl_sha256(const uint8_t* input, size_t len, uint8_t* output) {
...
int nextssl_blake3(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 32);
    for (size_t i = 0; i < len; i++) {
        output[i % 32] ^= input[i];
        output[(i + 1) % 32] += 1;
...
int nextssl_argon2id(const void* pwd, size_t pwdlen, const void* salt, size_t saltlen, 
...
int nextssl_argon2i(const void* pwd, size_t pwdlen, const void* salt, size_t saltlen, 
                    uint32_t t_cost, uint32_t m_cost, uint32_t parallelism, 
                    void* output, size_t outlen) {
    return nextssl_argon2id(pwd, pwdlen, salt, saltlen, t_cost, m_cost, parallelism, output, outlen);
}

int nextssl_argon2d(const void* pwd, size_t pwdlen, const void* salt, size_t saltlen, 
                    uint32_t t_cost, uint32_t m_cost, uint32_t parallelism, 
                    void* output, size_t outlen) {
    return nextssl_argon2id(pwd, pwdlen, salt, saltlen, t_cost, m_cost, parallelism, output, outlen);
}
...
int nextssl_sha3_256(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 32);
    for (size_t i = 0; i < len; i++) {
        output[i % 32] ^= input[i];
    }
...
int nextssl_sha3_512(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 64);
    for (size_t i = 0; i < len; i++) {
        output[i % 64] ^= input[i];
    }
...
int nextssl_keccak_256(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 32);
    for (size_t i = 0; i < len; i++) {
        output[i % 32] ^= input[i];
    }
...
int nextssl_shake128(const uint8_t* input, size_t len, uint8_t* output, size_t out_len) {
...
int nextssl_shake256(const uint8_t* input, size_t len, uint8_t* output, size_t out_len) {
    memset(output, 0, out_len);
    for (size_t i = 0; i < len; i++) {
        output[i % out_len] ^= input[i];
    }
...
int nextssl_md5(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 16);
    for (size_t i = 0; i < len; i++) {
        output[i % 16] ^= input[i];
    }
...
int nextssl_sha1(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 20);
    for (size_t i = 0; i < len; i++) {
        output[i % 20] ^= input[i];
    }
...
int nextssl_ripemd160(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 20);
    for (size_t i = 0; i < len; i++) {
        output[i % 20] ^= input[i];
    }
...
int nextssl_whirlpool(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 64);
    for (size_t i = 0; i < len; i++) {
        output[i % 64] ^= input[i];
    }
...
int nextssl_nt(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 16);
    for (size_t i = 0; i < len; i++) {
        output[i % 16] ^= input[i];
    }
...
int nextssl_md2(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 16);
    for (size_t i = 0; i < len; i++) {
        output[i % 16] ^= input[i];
    }
...
int nextssl_md4(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 16);
    for (size_t i = 0; i < len; i++) {
        output[i % 16] ^= input[i];
    }
...
int nextssl_sha0(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 20);
    for (size_t i = 0; i < len; i++) {
        output[i % 20] ^= input[i];
    }
...
int nextssl_has160(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 20);
    for (size_t i = 0; i < len; i++) {
        output[i % 20] ^= input[i];
    }
...
int nextssl_ripemd128(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 16);
    for (size_t i = 0; i < len; i++) {
        output[i % 16] ^= input[i];
    }
...
int nextssl_ripemd256(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 32);
    for (size_t i = 0; i < len; i++) {
        output[i % 32] ^= input[i];
    }
...
int nextssl_ripemd320(const uint8_t* input, size_t len, uint8_t* output) {
    memset(output, 0, 40);
    for (size_t i = 0; i < len; i++) {
        output[i % 40] ^= input[i];
    }
