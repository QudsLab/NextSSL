...
/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_compress
*
...
*              by NEXTSSL_MLKEM1024_AVX2_poly_reduce().
*
* Arguments:   - uint8_t *r: pointer to output byte array
*                            (of length KYBER_POLYCOMPRESSEDBYTES)
*              - const poly *a: pointer to input polynomial
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_compress(uint8_t r[160], const poly *restrict a) {
    size_t i;
    __m256i f0, f1;
    __m128i t0, t1;
    const __m256i v = _mm256_load_si256(&NEXTSSL_MLKEM1024_AVX2_qdata.vec[_16XV / 16]);
...
void NEXTSSL_MLKEM1024_AVX2_poly_decompress(poly *restrict r, const uint8_t a[160]) {
    unsigned int i;
    __m128i t;
    __m256i f;
    int16_t ti;
    const __m256i q = _mm256_load_si256(&NEXTSSL_MLKEM1024_AVX2_qdata.vec[_16XQ / 16]);
...
/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_tobytes
*
* Description: Serialization of a polynomial in NTT representation.
*              The coefficients of the input polynomial are assumed to
*              lie in the interval [0,q], i.e. the polynomial must be reduced
*              by NEXTSSL_MLKEM1024_AVX2_poly_reduce(). The coefficients are orderd as output by
*              NEXTSSL_MLKEM1024_AVX2_poly_ntt(); the serialized output coefficients are in bitreversed
*              order.
*
* Arguments:   - uint8_t *r: pointer to output byte array
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_tobytes(uint8_t r[KYBER_POLYBYTES], const poly *a) {
    NEXTSSL_MLKEM1024_AVX2_ntttobytes_avx(r, a->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_frombytes
*
* Description: De-serialization of a polynomial;
*              inverse of NEXTSSL_MLKEM1024_AVX2_poly_tobytes
*
* Arguments:   - poly *r: pointer to output polynomial
*              - const uint8_t *a: pointer to input byte array
*                                  (of KYBER_POLYBYTES bytes)
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_frombytes(poly *r, const uint8_t a[KYBER_POLYBYTES]) {
    NEXTSSL_MLKEM1024_AVX2_nttfrombytes_avx(r->vec, a, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_frommsg
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_frommsg(poly *restrict r, const uint8_t msg[KYBER_INDCPA_MSGBYTES]) {
...
/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_tomsg
*
...
*              - poly *a: pointer to input polynomial
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_tomsg(uint8_t msg[KYBER_INDCPA_MSGBYTES], const poly *restrict a) {
    unsigned int i;
    uint32_t small;
    __m256i f0, f1, g0, g1;
...
/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_getnoise_eta1
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_getnoise_eta1(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(KYBER_ETA1 * KYBER_N / 4 + 32) buf; // +32 bytes as required by NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1
    prf(buf.coeffs, KYBER_ETA1 * KYBER_N / 4, seed, nonce);
    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1(r, buf.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_getnoise_eta2
*
* Description: Sample a polynomial deterministically from a seed and a nonce,
*              with output polynomial close to centered binomial distribution
...
*              - uint8_t nonce: one-byte input nonce
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_getnoise_eta2(poly *r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce) {
    ALIGNED_UINT8(KYBER_ETA2 * KYBER_N / 4) buf;
    prf(buf.coeffs, KYBER_ETA2 * KYBER_N / 4, seed, nonce);
    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta2(r, buf.vec);
}

#define NOISE_NBLOCKS ((KYBER_ETA1*KYBER_N/4+SHAKE256_RATE-1)/SHAKE256_RATE)
void NEXTSSL_MLKEM1024_AVX2_poly_getnoise_eta1_4x(poly *r0,
        poly *r1,
        poly *r2,
        poly *r3,
    ...
    NEXTSSL_MLKEM1024_AVX2_shake256x4_absorb_once(&state, buf[0].coeffs, buf[1].coeffs, buf[2].coeffs, buf[3].coeffs, 33);
    NEXTSSL_MLKEM1024_AVX2_shake256x4_squeezeblocks(buf[0].coeffs, buf[1].coeffs, buf[2].coeffs, buf[3].coeffs, NOISE_NBLOCKS, &state);

    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1(r0, buf[0].vec);
    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1(r1, buf[1].vec);
    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1(r2, buf[2].vec);
    NEXTSSL_MLKEM1024_AVX2_poly_cbd_eta1(r3, buf[3].vec);
}


/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_ntt
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_ntt(poly *r) {
    NEXTSSL_MLKEM1024_AVX2_ntt_avx(r->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_invntt_tomont
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_invntt_tomont(poly *r) {
    NEXTSSL_MLKEM1024_AVX2_invntt_avx(r->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

void NEXTSSL_MLKEM1024_AVX2_poly_nttunpack(poly *r) {
    NEXTSSL_MLKEM1024_AVX2_nttunpack_avx(r->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_basemul_montgomery
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_basemul_montgomery(poly *r, const poly *a, const poly *b) {
    NEXTSSL_MLKEM1024_AVX2_basemul_avx(r->vec, a->vec, b->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_tomont
*
...
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_tomont(poly *r) {
    NEXTSSL_MLKEM1024_AVX2_tomont_avx(r->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_reduce
*
...
* Arguments:   - poly *r: pointer to input/output polynomial
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_reduce(poly *r) {
    NEXTSSL_MLKEM1024_AVX2_reduce_avx(r->vec, NEXTSSL_MLKEM1024_AVX2_qdata.vec);
}

/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_add
*
...
*            - const poly *b: pointer to second input polynomial
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_add(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    __m256i f0, f1;
...
/*************************************************
* Name:        NEXTSSL_MLKEM1024_AVX2_poly_sub
*
...
*            - const poly *b: pointer to second input polynomial
**************************************************/
void NEXTSSL_MLKEM1024_AVX2_poly_sub(poly *r, const poly *a, const poly *b) {
    unsigned int i;
    __m256i f0, f1;
