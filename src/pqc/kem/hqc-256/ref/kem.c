...
int NEXTSSL_HQC256_crypto_kem_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    NEXTSSL_HQC256_hqc_pke_keygen(pk, sk);
    return 0;
}
...
int NEXTSSL_HQC256_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
    ...
    NEXTSSL_HQC256_shake256_512_ds(&shake256state, theta, tmp, VEC_K_SIZE_BYTES + PUBLIC_KEY_BYTES + SALT_SIZE_BYTES, G_FCT_DOMAIN);

    // Encrypting m
    NEXTSSL_HQC256_hqc_pke_encrypt(u, v, m, theta, pk);

    // Computing shared secret
    memcpy(mc, m, VEC_K_SIZE_BYTES);
    NEXTSSL_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES, VEC_N_SIZE_BYTES, u, VEC_N_SIZE_64);
    NEXTSSL_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES, VEC_N1N2_SIZE_BYTES, v, VEC_N1N2_SIZE_64);
    NEXTSSL_HQC256_shake256_512_ds(&shake256state, ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES, K_FCT_DOMAIN);

    // Computing ciphertext
    NEXTSSL_HQC256_hqc_ciphertext_to_string(ct, u, v, salt);
...
int NEXTSSL_HQC256_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    NEXTSSL_HQC256_hqc_ciphertext_from_string(u, v, salt, ct);

    // Decrypting
    result = NEXTSSL_HQC256_hqc_pke_decrypt(m, sigma, u, v, sk);
    ...
    // Encrypting m'
    NEXTSSL_HQC256_hqc_pke_encrypt(u2, v2, m, theta, pk);

    // Check if c != c'
    result |= NEXTSSL_HQC256_vect_compare((uint8_t *)u, (uint8_t *)u2, VEC_N_SIZE_BYTES);
    result |= NEXTSSL_HQC256_vect_compare((uint8_t *)v, (uint8_t *)v2, VEC_N1N2_SIZE_BYTES);
