...
void NEXTSSL_HQC256_load8_arr(uint64_t *out64, size_t outlen, const uint8_t *in8, size_t inlen) {
    size_t index_in = 0;
    size_t index_out = 0;
...
void NEXTSSL_HQC256_store8_arr(uint8_t *out8, size_t outlen, const uint64_t *in64, size_t inlen) {
 ...
 */
void NEXTSSL_HQC256_hqc_secret_key_to_string(uint8_t *sk, const uint8_t *sk_seed, const uint8_t *sigma, const uint8_t *pk) {
 ...
 */
void NEXTSSL_HQC256_hqc_secret_key_from_string(uint64_t *x, uint64_t *y, uint8_t *sigma, uint8_t *pk, const uint8_t *sk) {
    seedexpander_state sk_seedexpander;

    memcpy(sigma, sk + SEED_BYTES, VEC_K_SIZE_BYTES);
    NEXTSSL_HQC256_seedexpander_init(&sk_seedexpander, sk, SEED_BYTES);
 ...
 */
void NEXTSSL_HQC256_hqc_public_key_to_string(uint8_t *pk, const uint8_t *pk_seed, const uint64_t *s) {
    memcpy(pk, pk_seed, SEED_BYTES);
    NEXTSSL_HQC256_store8_arr(pk + SEED_BYTES, VEC_N_SIZE_BYTES, s, VEC_N_SIZE_64);
}
 ...
 * @param[in] pk String containing the public key
 */
void NEXTSSL_HQC256_hqc_public_key_from_string(uint64_t *h, uint64_t *s, const uint8_t *pk) {
    seedexpander_state pk_seedexpander;

    NEXTSSL_HQC256_seedexpander_init(&pk_seedexpander, pk, SEED_BYTES);
    NEXTSSL_HQC256_vect_set_random(&pk_seedexpander, h);

    NEXTSSL_HQC256_load8_arr(s, VEC_N_SIZE_64, pk + SEED_BYTES, VEC_N_SIZE_BYTES);
 ...
 */
void NEXTSSL_HQC256_hqc_ciphertext_to_string(uint8_t *ct, const uint64_t *u, const uint64_t *v, const uint8_t *salt) {
    NEXTSSL_HQC256_store8_arr(ct, VEC_N_SIZE_BYTES, u, VEC_N_SIZE_64);
    NEXTSSL_HQC256_store8_arr(ct + VEC_N_SIZE_BYTES, VEC_N1N2_SIZE_BYTES, v, VEC_N1N2_SIZE_64);
 ...
 */
void NEXTSSL_HQC256_hqc_ciphertext_from_string(uint64_t *u, uint64_t *v, uint8_t *salt, const uint8_t *ct) {
    NEXTSSL_HQC256_load8_arr(u, VEC_N_SIZE_64, ct, VEC_N_SIZE_BYTES);
    NEXTSSL_HQC256_load8_arr(v, VEC_N1N2_SIZE_64, ct + VEC_N_SIZE_BYTES, VEC_N1N2_SIZE_BYTES);
