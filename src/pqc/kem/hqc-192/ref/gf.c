 ...
 */
uint16_t NEXTSSL_HQC192_gf_mul(uint16_t a, uint16_t b) {
    uint8_t c[2] = {0};
 ...
 */
uint16_t NEXTSSL_HQC192_gf_square(uint16_t a) {
    uint32_t b = a;
    uint32_t s = b & 1;
 ...
 * @param[in] a Element of GF(2^PARAM_M)
 */
uint16_t NEXTSSL_HQC192_gf_inverse(uint16_t a) {
    uint16_t inv = a;
    uint16_t tmp1, tmp2;

    inv = NEXTSSL_HQC192_gf_square(a); /* a^2 */
    tmp1 = NEXTSSL_HQC192_gf_mul(inv, a); /* a^3 */
    inv = NEXTSSL_HQC192_gf_square(inv); /* a^4 */
    tmp2 = NEXTSSL_HQC192_gf_mul(inv, tmp1); /* a^7 */
    tmp1 = NEXTSSL_HQC192_gf_mul(inv, tmp2); /* a^11 */
    inv = NEXTSSL_HQC192_gf_mul(tmp1, inv); /* a^15 */
    inv = NEXTSSL_HQC192_gf_square(inv); /* a^30 */
    inv = NEXTSSL_HQC192_gf_square(inv); /* a^60 */
    inv = NEXTSSL_HQC192_gf_square(inv); /* a^120 */
    inv = NEXTSSL_HQC192_gf_mul(inv, tmp2); /* a^127 */
    inv = NEXTSSL_HQC192_gf_square(inv); /* a^254 */
    return inv;
}
