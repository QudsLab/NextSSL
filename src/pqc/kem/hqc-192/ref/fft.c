 ...
 *
 * This function is a subroutine of the function NEXTSSL_HQC192_fft.
 *
 * @param[out] w Array
 * @param[in] f Array
...
static void fft_rec(uint16_t *w, uint16_t *f, size_t f_coeffs, uint8_t m, uint32_t m_f, const uint16_t *betas) {
    ...
    if (m_f == 1) {
        for (i = 0; i < m; ++i) {
            tmp[i] = NEXTSSL_HQC192_gf_mul(betas[i], f[1]);
        }
    ...
    if (betas[m - 1] != 1) {
        ...
        x <<= m_f;
        for (i = 1; i < x; ++i) {
            beta_m_pow = NEXTSSL_HQC192_gf_mul(beta_m_pow, betas[m - 1]);
            f[i] = NEXTSSL_HQC192_gf_mul(beta_m_pow, f[i]);
        }
    }
    ...
    for (i = 0; i + 1 < m; ++i) {
        gammas[i] = NEXTSSL_HQC192_gf_mul(betas[i], NEXTSSL_HQC192_gf_inverse(betas[m - 1]));
        deltas[i] = NEXTSSL_HQC192_gf_square(gammas[i]) ^ gammas[i];
    }
    ...
    if (f_coeffs <= 3) { // 3-coefficient polynomial f case: f1 is constant
        ...
        w[k] = u[0] ^ f1[0];
        for (i = 1; i < k; ++i) {
            w[i] = u[i] ^ NEXTSSL_HQC192_gf_mul(gammas_sums[i], f1[0]);
            w[k + i] = w[i] ^ f1[0];
        }
    } else {
        ...
        w[k] ^= u[0];
        for (i = 1; i < k; ++i) {
            w[i] = u[i] ^ NEXTSSL_HQC192_gf_mul(gammas_sums[i], v[i]);
            w[k + i] ^= w[i];
        }
    }
}
 ...
 */
void NEXTSSL_HQC192_fft(uint16_t *w, const uint16_t *f, size_t f_coeffs) {
    ...
    for (i = 0; i < PARAM_M - 1; ++i) {
        deltas[i] = NEXTSSL_HQC192_gf_square(betas[i]) ^ betas[i];
    }
    ...
    // Find other roots
    for (i = 1; i < k; ++i) {
        w[i] = u[i] ^ NEXTSSL_HQC192_gf_mul(betas_sums[i], v[i]);
        w[k + i] ^= w[i];
    }
}
 ...
 */
void NEXTSSL_HQC192_fft_retrieve_error_poly(uint8_t *error, const uint16_t *w) {
