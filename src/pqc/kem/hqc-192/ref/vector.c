 ...
 */
void NEXTSSL_HQC192_vect_set_random_fixed_weight(seedexpander_state *ctx, uint64_t *v, uint16_t weight) {
    ...
    NEXTSSL_HQC192_seedexpander(ctx, rand_bytes, 4 * weight);
 ...
 * array of bytes using the NEXTSSL_HQC192_seedexpander function, and drop the extra bits using a mask.
 *
 * @param[in] v Pointer to an array
 * @param[in] ctx Pointer to the context of the seed expander
 */
void NEXTSSL_HQC192_vect_set_random(seedexpander_state *ctx, uint64_t *v) {
    uint8_t rand_bytes[VEC_N_SIZE_BYTES] = {0};

    NEXTSSL_HQC192_seedexpander(ctx, rand_bytes, VEC_N_SIZE_BYTES);

    NEXTSSL_HQC192_load8_arr(v, VEC_N_SIZE_64, rand_bytes, VEC_N_SIZE_BYTES);
 ...
 */
void NEXTSSL_HQC192_vect_add(uint64_t *o, const uint64_t *v1, const uint64_t *v2, size_t size) {
 ...
 */
uint8_t NEXTSSL_HQC192_vect_compare(const uint8_t *v1, const uint8_t *v2, size_t size) {
    uint16_t r = 0x0100;
 ...
 */
void NEXTSSL_HQC192_vect_resize(uint64_t *o, uint32_t size_o, const uint64_t *v, uint32_t size_v) {
