 ...
 */
void NEXTSSL_HQC128_code_encode(uint64_t *em, const uint8_t *m) {
    uint8_t tmp[VEC_N1_SIZE_BYTES] = {0};

    NEXTSSL_HQC128_reed_solomon_encode(tmp, m);
    NEXTSSL_HQC128_reed_muller_encode(em, tmp);
 ...
 */
void NEXTSSL_HQC128_code_decode(uint8_t *m, const uint64_t *em) {
    uint8_t tmp[VEC_N1_SIZE_BYTES] = {0};

    NEXTSSL_HQC128_reed_muller_decode(tmp, em);
    NEXTSSL_HQC128_reed_solomon_decode(m, tmp);
