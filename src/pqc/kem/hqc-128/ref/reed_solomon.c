 ...
 */
void NEXTSSL_HQC128_reed_solomon_encode(uint8_t *cdw, const uint8_t *msg) {
    ...
    for (size_t i = 0; i < PARAM_K; ++i) {
        ...
        for (size_t j = 0; j < PARAM_G; ++j) {
            tmp[j] = NEXTSSL_HQC128_gf_mul(gate_value, PARAM_RS_POLY[j]);
        }
...
static void compute_syndromes(uint16_t *syndromes, uint8_t *cdw) {
    for (size_t i = 0; i < 2 * PARAM_DELTA; ++i) {
        for (size_t j = 1; j < PARAM_N1; ++j) {
            syndromes[i] ^= NEXTSSL_HQC128_gf_mul(cdw[j], alpha_ij_pow[i][j - 1]);
        }
        syndromes[i] ^= cdw[0];
    }
}
...
static uint16_t compute_elp(uint16_t *sigma, const uint16_t *syndromes) {
    ...
    for (mu = 0; (mu < (2 * PARAM_DELTA)); ++mu) {
        ...
        dd = NEXTSSL_HQC128_gf_mul(d, NEXTSSL_HQC128_gf_inverse(d_p));

        for (i = 1; (i <= mu + 1) && (i <= PARAM_DELTA); ++i) {
            sigma[i] ^= NEXTSSL_HQC128_gf_mul(dd, X_sigma_p[i]);
        }
        ...
        for (i = 1; (i <= mu + 1) && (i <= PARAM_DELTA); ++i) {
            d ^= NEXTSSL_HQC128_gf_mul(sigma[i], syndromes[mu + 1 - i]);
        }
    }
 ...
 *
 * See function NEXTSSL_HQC128_fft for more details.
 *
...
static void compute_roots(uint8_t *error, uint16_t *sigma) {
    ...
    NEXTSSL_HQC128_fft(w, sigma, PARAM_DELTA + 1);
    NEXTSSL_HQC128_fft_retrieve_error_poly(error, w);
}
...
static void compute_z_poly(uint16_t *z, const uint16_t *sigma, uint16_t degree, const uint16_t *syndromes) {
    ...
    for (i = 2; i <= PARAM_DELTA; ++i) {
        ...
        for (j = 1; j < i; ++j) {
            z[i] ^= mask & NEXTSSL_HQC128_gf_mul(sigma[j], syndromes[i - j - 1]);
        }
    }
}
...
static void compute_error_values(uint16_t *error_values, const uint16_t *z, const uint8_t *error) {
    ...
    for (size_t i = 0; i < PARAM_DELTA; ++i) {
        tmp1 = 1;
        tmp2 = 1;
        inverse = NEXTSSL_HQC128_gf_inverse(beta_j[i]);
        inverse_power_j = 1;

        for (size_t j = 1; j <= PARAM_DELTA; ++j) {
            inverse_power_j = NEXTSSL_HQC128_gf_mul(inverse_power_j, inverse);
            tmp1 ^= NEXTSSL_HQC128_gf_mul(inverse_power_j, z[j]);
        }
        for (size_t k = 1; k < PARAM_DELTA; ++k) {
            tmp2 = NEXTSSL_HQC128_gf_mul(tmp2, (1 ^ NEXTSSL_HQC128_gf_mul(inverse, beta_j[(i + k) % PARAM_DELTA])));
        }
        mask1 = (uint16_t) (((int16_t) i - delta_real_value) >> 15); // i < delta_real_value
        e_j[i] = mask1 & NEXTSSL_HQC128_gf_mul(tmp1, NEXTSSL_HQC128_gf_inverse(tmp2));
    }
 ...
 */
void NEXTSSL_HQC128_reed_solomon_decode(uint8_t *msg, uint8_t *cdw) {
