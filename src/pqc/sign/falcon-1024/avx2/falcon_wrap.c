...
/* see api.h */
int
NEXTSSL_FALCON1024_AVX2_crypto_sign_keypair(
    ...
    NEXTSSL_FALCON1024_AVX2_keygen(&rng, f, g, F, NULL, h, 10, tmp.b);
    ...
    sk[0] = 0x50 + 10;
    u = 1;
    v = NEXTSSL_FALCON1024_AVX2_trim_i8_encode(
            sk + u, NEXTSSL_FALCON1024_AVX2_CRYPTO_SECRETKEYBYTES - u,
            f, 10, NEXTSSL_FALCON1024_AVX2_max_fg_bits[10]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = NEXTSSL_FALCON1024_AVX2_trim_i8_encode(
            sk + u, NEXTSSL_FALCON1024_AVX2_CRYPTO_SECRETKEYBYTES - u,
            g, 10, NEXTSSL_FALCON1024_AVX2_max_fg_bits[10]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != NEXTSSL_FALCON1024_AVX2_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
     ...
     */
    pk[0] = 0x00 + 10;
    v = NEXTSSL_FALCON1024_AVX2_modq_encode(
            pk + 1, NEXTSSL_FALCON1024_AVX2_CRYPTO_PUBLICKEYBYTES - 1,
            h, 10);
    if (v != NEXTSSL_FALCON1024_AVX2_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    u = 1;
    v = NEXTSSL_FALCON1024_AVX2_trim_i8_decode(
            f, 10, NEXTSSL_FALCON1024_AVX2_max_fg_bits[10],
            sk + u, NEXTSSL_FALCON1024_AVX2_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!NEXTSSL_FALCON1024_AVX2_complete_private(G, f, g, F, 10, tmp.b)) {
        return -1;
    }
    ...
    NEXTSSL_FALCON1024_AVX2_hash_to_point_ct(&sc, r.hm, 10, tmp.b);
     ...
     */
    NEXTSSL_FALCON1024_AVX2_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 10, tmp.b);
    v = NEXTSSL_FALCON1024_AVX2_comp_encode(sigbuf, *sigbuflen, r.sig, 10);
    if (v != 0) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
...
do_verify(
    ...
    if (NEXTSSL_FALCON1024_AVX2_modq_decode(h, 10,
                                            pk + 1, NEXTSSL_FALCON1024_AVX2_CRYPTO_PUBLICKEYBYTES - 1)
            != NEXTSSL_FALCON1024_AVX2_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
    NEXTSSL_FALCON1024_AVX2_to_ntt_monty(h, 10);
    ...
    v = NEXTSSL_FALCON1024_AVX2_comp_decode(sig, 10, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    NEXTSSL_FALCON1024_AVX2_hash_to_point_ct(&sc, hm, 10, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!NEXTSSL_FALCON1024_AVX2_verify_raw(hm, sig, h, 10, tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
NEXTSSL_FALCON1024_AVX2_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = NEXTSSL_FALCON1024_AVX2_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
NEXTSSL_FALCON1024_AVX2_crypto_sign_verify(
...
/* see api.h */
int
NEXTSSL_FALCON1024_AVX2_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = NEXTSSL_FALCON1024_AVX2_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
NEXTSSL_FALCON1024_AVX2_crypto_sign_open(
