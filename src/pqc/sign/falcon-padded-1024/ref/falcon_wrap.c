...
/* see api.h */
int
NEXTSSL_FALCONPADDED1024_crypto_sign_keypair(
    ...
    NEXTSSL_FALCONPADDED1024_keygen(&rng, f, g, F, NULL, h, 10, tmp.b);
    ...
    sk[0] = 0x50 + 10;
    u = 1;
    v = NEXTSSL_FALCONPADDED1024_trim_i8_encode(
            sk + u, NEXTSSL_FALCONPADDED1024_CRYPTO_SECRETKEYBYTES - u,
            f, 10, NEXTSSL_FALCONPADDED1024_max_fg_bits[10]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = NEXTSSL_FALCONPADDED1024_trim_i8_encode(
            sk + u, NEXTSSL_FALCONPADDED1024_CRYPTO_SECRETKEYBYTES - u,
            g, 10, NEXTSSL_FALCONPADDED1024_max_fg_bits[10]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != NEXTSSL_FALCONPADDED1024_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
     ...
     */
    pk[0] = 0x00 + 10;
    v = NEXTSSL_FALCONPADDED1024_modq_encode(
            pk + 1, NEXTSSL_FALCONPADDED1024_CRYPTO_PUBLICKEYBYTES - 1,
            h, 10);
    if (v != NEXTSSL_FALCONPADDED1024_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t sigbuflen,
    ...
    u = 1;
    v = NEXTSSL_FALCONPADDED1024_trim_i8_decode(
            f, 10, NEXTSSL_FALCONPADDED1024_max_fg_bits[10],
            sk + u, NEXTSSL_FALCONPADDED1024_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!NEXTSSL_FALCONPADDED1024_complete_private(G, f, g, F, 10, tmp.b)) {
        return -1;
    }
    ...
    NEXTSSL_FALCONPADDED1024_hash_to_point_ct(&sc, r.hm, 10, tmp.b);
     ...
     */
    for (;;) {
        NEXTSSL_FALCONPADDED1024_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, 10, tmp.b);
        v = NEXTSSL_FALCONPADDED1024_comp_encode(sigbuf, sigbuflen, r.sig, 10);
        if (v != 0) {
            inner_shake256_ctx_release(&sc);
...
do_verify(
    ...
    if (NEXTSSL_FALCONPADDED1024_modq_decode(h, 10,
            pk + 1, NEXTSSL_FALCONPADDED1024_CRYPTO_PUBLICKEYBYTES - 1)
            != NEXTSSL_FALCONPADDED1024_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
    NEXTSSL_FALCONPADDED1024_to_ntt_monty(h, 10);
    ...
    v = NEXTSSL_FALCONPADDED1024_comp_decode(sig, 10, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
        if (sigbuflen == NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES - NONCELEN - 1) {
            while (v < sigbuflen) {
                if (sigbuf[v++] != 0) {
                    return -1;
                }
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    NEXTSSL_FALCONPADDED1024_hash_to_point_ct(&sc, hm, 10, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!NEXTSSL_FALCONPADDED1024_verify_raw(hm, sig, h, 10, tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
NEXTSSL_FALCONPADDED1024_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;

    vlen = NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
NEXTSSL_FALCONPADDED1024_crypto_sign_verify(
...
/* see api.h */
int
NEXTSSL_FALCONPADDED1024_crypto_sign(
     ...
     */
    memmove(sm + NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES, m, mlen);
    sigbuf = sm + 1 + NONCELEN;
    sigbuflen = NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
NEXTSSL_FALCONPADDED1024_crypto_sign_open(
    uint8_t *m, size_t *mlen,
    const uint8_t *sm, size_t smlen, const uint8_t *pk) {
    const uint8_t *sigbuf;
    size_t pmlen, sigbuflen;

    if (smlen < NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES) {
        return -1;
    }
    sigbuflen = NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES - NONCELEN - 1;
    pmlen = smlen - NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES;
    if (sm[0] != 0x30 + 10) {
        return -1;
    }
    sigbuf = sm + 1 + NONCELEN;
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES, pmlen, pk) < 0) {
        return -1;
    }
     ...
     * overlaps.
     */
    memmove(m, sm + NEXTSSL_FALCONPADDED1024_CRYPTO_BYTES, pmlen);
    *mlen = pmlen;
    return 0;
}
