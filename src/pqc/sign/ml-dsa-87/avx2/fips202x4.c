...
static void keccakx4_absorb_once(__m256i s[25],
    ...
    while (inlen >= r) {
        ...
        NEXTSSL_MLDSA87_AVX2_f1600x4(s, KeccakF_RoundConstants);
    }
...
void NEXTSSL_MLDSA87_AVX2_shake128x4_absorb_once(keccakx4_state *state,
        const uint8_t *in0,
        const uint8_t *in1,
        const uint8_t *in2,
        const uint8_t *in3,
...
void NEXTSSL_MLDSA87_AVX2_shake128x4_squeezeblocks(uint8_t *out0,
        uint8_t *out1,
        uint8_t *out2,
        uint8_t *out3,
        size_t nblocks,
...
void NEXTSSL_MLDSA87_AVX2_shake256x4_absorb_once(keccakx4_state *state,
        const uint8_t *in0,
        const uint8_t *in1,
        const uint8_t *in2,
        const uint8_t *in3,
...
void NEXTSSL_MLDSA87_AVX2_shake256x4_squeezeblocks(uint8_t *out0,
        uint8_t *out1,
        uint8_t *out2,
        uint8_t *out3,
        size_t nblocks,
...
void NEXTSSL_MLDSA87_AVX2_shake128x4(uint8_t *out0,
                                     uint8_t *out1,
                                     uint8_t *out2,
                                     uint8_t *out3,
                                     size_t outlen,
    ...
    NEXTSSL_MLDSA87_AVX2_shake128x4_absorb_once(&state, in0, in1, in2, in3, inlen);
    NEXTSSL_MLDSA87_AVX2_shake128x4_squeezeblocks(out0, out1, out2, out3, nblocks, &state);
    ...
    if (outlen) {
        NEXTSSL_MLDSA87_AVX2_shake128x4_squeezeblocks(t[0], t[1], t[2], t[3], 1, &state);
        for (i = 0; i < outlen; ++i) {
            out0[i] = t[0][i];
            out1[i] = t[1][i];
            out2[i] = t[2][i];
...
void NEXTSSL_MLDSA87_AVX2_shake256x4(uint8_t *out0,
                                     uint8_t *out1,
                                     uint8_t *out2,
                                     uint8_t *out3,
                                     size_t outlen,
    ...
    NEXTSSL_MLDSA87_AVX2_shake256x4_absorb_once(&state, in0, in1, in2, in3, inlen);
    NEXTSSL_MLDSA87_AVX2_shake256x4_squeezeblocks(out0, out1, out2, out3, nblocks, &state);
    ...
    if (outlen) {
        NEXTSSL_MLDSA87_AVX2_shake256x4_squeezeblocks(t[0], t[1], t[2], t[3], 1, &state);
        for (i = 0; i < outlen; ++i) {
            out0[i] = t[0][i];
            out1[i] = t[1][i];
            out2[i] = t[2][i];
