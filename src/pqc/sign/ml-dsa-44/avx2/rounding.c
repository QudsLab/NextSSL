...
*
**************************************************/
void NEXTSSL_MLDSA44_AVX2_power2round_avx(__m256i *a1, __m256i *a0, const __m256i *a) {
    unsigned int i;
    __m256i f, f0, f1;
...
*
**************************************************/
void NEXTSSL_MLDSA44_AVX2_decompose_avx(__m256i *a1, __m256i *a0, const __m256i *a) {
    unsigned int i;
    __m256i f, f0, f1, t;
...
**************************************************/
unsigned int NEXTSSL_MLDSA44_AVX2_make_hint_avx(uint8_t hint[N], const __m256i *restrict a0, const __m256i *restrict a1) {
    unsigned int i, n = 0;
    __m256i f0, f1, g0, g1;
    uint32_t bad;
    uint64_t idx;
    ...
    for (i = 0; i < N / 8; ++i) {
        ...
        memcpy(&idx, NEXTSSL_MLDSA44_AVX2_idxlut[bad], 8);
...
*
**************************************************/
void NEXTSSL_MLDSA44_AVX2_use_hint_avx(__m256i *b, const __m256i *a, const __m256i *restrict hint) {
    unsigned int i;
    __m256i a0[N / 8];
    __m256i f, g, h, t;
    const __m256i zero = _mm256_setzero_si256();
    ...
    NEXTSSL_MLDSA44_AVX2_decompose_avx(b, a0, a);
    for (i = 0; i < N / 8; i++) {
