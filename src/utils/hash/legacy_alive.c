...
int nextssl_ripemd160(const uint8_t *msg, size_t len, uint8_t *out) {
    ripemd160_hash(msg, len, out);
    return 0;
}

int nextssl_whirlpool(const uint8_t *msg, size_t len, uint8_t *out) {
    whirlpool_hash(msg, len, out);
    return 0;
}

int nextssl_nt_hash(const char *password, uint8_t *out) {
    nt_hash(password, out);
    return 0;
}

int nextssl_aes_ecb_encrypt(const uint8_t* key, const uint8_t* pntxt, size_t ptextLen, uint8_t* crtxt) {
