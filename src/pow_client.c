...
#include <string.h>
#include <stdint.h>
#include "pow_export.h"

// Include all hash headers
#include "BaseHash/md2/md2.h"
#include "BaseHash/md4/md4.h"
#include "BaseHash/md5/md5.h"
#include "BaseHash/sha0/sha0.h"
#include "BaseHash/sha1/sha1.h"
#include "BaseHash/sha224/sha224.h"
#include "BaseHash/sha256/sha256.h"
#include "BaseHash/sha512/sha512.h"
#include "BaseHash/sha3/sha3.h"
#include "BaseHash/sha3_224/sha3_224.h"
#include "BaseHash/sha3_384/sha3_384.h"
#include "BaseHash/keccak/keccak.h"
#include "BaseHash/shake/shake.h"
#include "BaseHash/ripemd/ripemd160.h"
#include "BaseHash/ripemd128/ripemd128.h"
#include "BaseHash/ripemd256/ripemd256.h"
#include "BaseHash/ripemd320/ripemd320.h"
#include "BaseHash/blake2b/blake2b.h"
#include "BaseHash/blake2s/blake2s.h"
#include "BaseHash/whirlpool/whirlpool.h"
#include "BaseHash/has160/has160.h"
#include "BaseHash/nt/nt.h"
...
typedef struct {
    int nonce;
    uint8_t hash[128]; // Max digest size
    int hash_size;
} PoWResult;

// Multi-hash result structure
typedef struct {
    int nonce;
    uint8_t hashes[10][128]; // Support up to 10 hashes
    int hash_sizes[10];
    int num_hashes;
} MultiPoWResult;
...
EXPORT PoWResult generate_pow_single(const char *input, HashAlgorithm algo, int difficulty, int min_nonce, int max_nonce) {
    PoWResult result;
    result.nonce = -1;
    memset(result.hash, 0, 128);
    result.hash_size = 0;
...
// Generate PoW for multiple hash algorithms (all must pass)
EXPORT MultiPoWResult generate_pow_multi(const char *input, HashAlgorithm *algos, int num_algos, int difficulty, int min_nonce, int max_nonce) {
    MultiPoWResult result;
    result.nonce = -1;
    result.num_hashes = num_algos;
    memset(result.hashes, 0, sizeof(result.hashes));
    memset(result.hash_sizes, 0, sizeof(result.hash_sizes));
    ...
    for (int nonce = min_nonce; nonce <= max_nonce; nonce++) {
        ...
        uint8_t temp_hashes[10][128];
        int temp_sizes[10];
        
        // Check all algorithms
        for (int i = 0; i < num_algos; i++) {
            compute_hash(algos[i], (uint8_t*)combined, len + n, temp_hashes[i], &temp_sizes[i]);
            
            if (!has_leading_zeros(temp_hashes[i], temp_sizes[i], difficulty)) {
                all_passed = 0;
                break;
            }
        }
        
        // If all passed, save results
        if (all_passed) {
            result.nonce = nonce;
            for (int i = 0; i < num_algos; i++) {
                memcpy(result.hashes[i], temp_hashes[i], temp_sizes[i]);
                result.hash_sizes[i] = temp_sizes[i];
            }
            break;
        }
    }
