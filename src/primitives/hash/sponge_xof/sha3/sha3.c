...
void sha3_final_custom(uint8_t *digest, SHA3_CTX *ctx, uint8_t suffix) {
    /* Padding: append suffix, then 0x00s, then 0x80 */
    memset(ctx->buffer + ctx->buf_len, 0, ctx->rate - ctx->buf_len);
    ctx->buffer[ctx->buf_len] = suffix;
...
void sha3_final(uint8_t *digest, SHA3_CTX *ctx) {
    sha3_final_custom(digest, ctx, 0x06);
}
