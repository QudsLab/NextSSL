...
int nextssl_profile_load(nextssl_profile_t profile, nextssl_config_t *config) {
    ...
    /* Universal defaults for all profiles */
    strncpy(config->default_salt, "NextSSL_Default_Salt",
            sizeof(config->default_salt) - 1);
#ifdef NEXTSSL_BUILD_LITE
    config->max_argon2_wu = 8;
    config->max_argon2_mu = 65536;
#else
    config->max_argon2_wu = 32;
    config->max_argon2_mu = 2097152;
#endif
    ...
    switch (profile) {
        case NEXTSSL_PROFILE_MODERN:
            ...
            config->argon2_params = (nextssl_argon2_params_t){4, 262144, 4, 32};
            break;
        ...
        case NEXTSSL_LEVEL_HIGH:
            /* High security: ML-KEM-768 + ML-DSA-65 + SHA3-256 + 512 MB Argon2 */
            config->default_hash = NEXTSSL_HASH_SHA3_256;
            config->default_aead = NEXTSSL_AEAD_AES_256_GCM;
            config->default_kdf  = NEXTSSL_KDF_ARGON2ID;
            config->default_sign = NEXTSSL_SIGN_ML_DSA_65;
            config->default_kem  = NEXTSSL_KEM_ML_KEM_768;
            config->default_pow  = NEXTSSL_POW_SHA3_256;
            config->strict_mode  = true;
            config->allow_legacy = false;
            config->pqc_only     = true;
            config->argon2_params = (nextssl_argon2_params_t){4, 524288, 4, 48};
            config->max_argon2_wu = 32;
            config->max_argon2_mu = 2097152;
            break;

        case NEXTSSL_LEVEL_MAX:
            /* Maximum security: ML-KEM-1024 + ML-DSA-87 + BLAKE3 + 1 GB Argon2 */
            config->default_hash = NEXTSSL_HASH_BLAKE3;
            config->default_aead = NEXTSSL_AEAD_AES_256_GCM;
            config->default_kdf  = NEXTSSL_KDF_ARGON2ID;
            config->default_sign = NEXTSSL_SIGN_ML_DSA_87;
            ...
            config->allow_legacy = false;
            config->pqc_only     = true;
            config->argon2_params = (nextssl_argon2_params_t){8, 1048576, 8, 64};
            config->max_argon2_wu = 32;
            config->max_argon2_mu = 2097152;
            break;
#endif
