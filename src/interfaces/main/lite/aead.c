...
#define NEXTSSL_AEAD_TAG_SIZE 16
#define NEXTSSL_AEAD_KEY_SIZE 32
#define NEXTSSL_AEAD_NONCE_SIZE 12

int nextssl_aead_encrypt(
...
int nextssl_aead_decrypt(
    ...
    if (ciphertext_len < NEXTSSL_AEAD_TAG_SIZE) {
        return -1;  // Too short to contain tag
    }
    
    size_t plaintext_len = ciphertext_len - NEXTSSL_AEAD_TAG_SIZE;
    ...
    if (strcmp(algorithm, "ChaCha20-Poly1305") == 0) {
        if (ChaCha20_Poly1305_decrypt(key, nonce, aad, aad_len,
                                     ciphertext, plaintext_len + NEXTSSL_AEAD_TAG_SIZE,
                                     plaintext) != 0) {
            return -5;
        }
...
int nextssl_aead_overhead(const char *algorithm) {
    // Both algorithms use 16-byte authentication tag
    return NEXTSSL_AEAD_TAG_SIZE;
}

int nextssl_aead_key_size(const char *algorithm) {
    return NEXTSSL_AEAD_KEY_SIZE;  // 32 bytes for both
}

int nextssl_aead_nonce_size(const char *algorithm) {
    return NEXTSSL_AEAD_NONCE_SIZE;  // 12 bytes for both
}
