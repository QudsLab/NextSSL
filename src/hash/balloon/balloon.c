...
#include <pthread.h>
#else
/* Minimal pthread compat for Windows */
#include <windows.h>
#include <process.h>
...
static int pthread_create(pthread_t *t, const pthread_attr_t *a,
    ...
    (void)a;
    *t = (HANDLE)_beginthreadex(NULL, 0,
        (unsigned (__stdcall *)(void *))fn, arg, 0, NULL);
    return *t ? 0 : -1;
}
...
options_validate (struct balloon_options *opts)
{
  if (opts->n_threads > THREADS_MAX)
    return BALLOON_NTHREADS_TOO_BIG;
    ...
    struct balloon_options *opts)
{
  ...
  if (!out || !in || !salt)
    return BALLOON_NULL_POINTER;

  if (inlen >= INLEN_MAX)
    return BALLOON_INLEN_TOO_BIG;
  ...
  for (uint32_t t = 0; t < opts->n_threads; t++) {
    ...
    if (thread_data[t].error != BALLOON_NONE)
