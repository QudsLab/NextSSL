...
static void shake_do_final(SHAKE_CTX *ctx, uint8_t pad_suffix) {
    ctx->buffer[ctx->buf_len++] = pad_suffix;
...
/* Standard SHAKE padding (0x1F) */
void shake_final(SHAKE_CTX *ctx) {
    shake_do_final(ctx, 0x1F);
}

/* cSHAKE / KMAC padding (0x04) — used by SP 800-185 constructions */
void shake_custom_final(SHAKE_CTX *ctx, uint8_t pad_suffix) {
    shake_do_final(ctx, pad_suffix);
}
