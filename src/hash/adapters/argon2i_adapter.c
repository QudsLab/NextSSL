/* argon2i_adapter.c — Argon2i KDF hash adapter (Plan 40002)
 * Identical to argon2id_adapter.c except calls argon2i_hash_raw(). */
...
static int do_hash(argon2i_impl_t *p,
                   ...
                   uint8_t *out, size_t out_len)
{
    ...
    if (p->salt) { s = p->salt; slen = p->salt_len; }
        ...
        return argon2i_hash_raw(p->iterations, p->memory, p->parallelism,
...
static int argon2i_hash_fn(void *impl, const uint8_t *d, size_t dl, uint8_t *o, size_t ol)
{ return do_hash((argon2i_impl_t *)impl, d, dl, o, ol); }

static void argon2i_init_fn   (void *impl) { ((argon2i_impl_t *)impl)->buf_len = 0; }
static void argon2i_update_fn (void *impl, const uint8_t *d, size_t l)
{
    argon2i_impl_t *p = (argon2i_impl_t *)impl;
    size_t room = sizeof(p->buf) - p->buf_len;
    if (l > room) l = room;
    memcpy(p->buf + p->buf_len, d, l); p->buf_len += l;
}
static void argon2i_final_fn  (void *impl, uint8_t *o, size_t ol)
{
    argon2i_impl_t *p = (argon2i_impl_t *)impl;
    do_hash(p, p->buf, p->buf_len, o, ol);
    secure_zero(p->buf, p->buf_len); p->buf_len = 0;
}
static void argon2i_destroy_fn(void *impl)
{
    argon2i_impl_t *p = (argon2i_impl_t *)impl;
...
hash_adapter_t *argon2i_adapter_create(void)
{
    ...
    argon2i_impl_t *p = (argon2i_impl_t *)malloc(sizeof(argon2i_impl_t));
    ...
    a->hash_fn = argon2i_hash_fn; a->init_fn = argon2i_init_fn;
    a->update_fn = argon2i_update_fn; a->final_fn = argon2i_final_fn;
    a->destroy_fn = argon2i_destroy_fn;
    return a;
}
...
{
    if (!a || !a->impl) return;
    argon2i_impl_t *p = (argon2i_impl_t *)a->impl;
