...
typedef struct {
    ...
    uint32_t key_length;
    uint8_t *salt;         /* NULL = random; heap-owned when non-NULL */
    size_t   salt_len;
    /* accumulator (for streaming init/update/final used by HMAC) */
    uint8_t  buf[2040];
    size_t   buf_len;
} argon2id_impl_t;

/* ── helpers ────────────────────────────────────────────────────────────── */
static int do_hash(argon2id_impl_t *p,
                   ...
                   uint8_t *out, size_t out_len)
{
    ...
    if (p->salt) {
        ...
        s    = tmp_salt;
        slen = sizeof(tmp_salt);
        return argon2id_hash_raw(p->iterations, p->memory, p->parallelism,
                                 data, data_len,
                                 s, slen,
...
/* ── hash_adapter_t function pointers ──────────────────────────────────── */
static int argon2id_hash_fn(void *impl,
                             const uint8_t *data, size_t data_len,
                             uint8_t *out, size_t out_len)
{ return do_hash((argon2id_impl_t *)impl, data, data_len, out, out_len); }

static void argon2id_init_fn(void *impl)
{
    argon2id_impl_t *p = (argon2id_impl_t *)impl;
    p->buf_len = 0;
}

static void argon2id_update_fn(void *impl, const uint8_t *data, size_t len)
{
    argon2id_impl_t *p = (argon2id_impl_t *)impl;
...
static void argon2id_final_fn(void *impl, uint8_t *out, size_t out_len)
{
    argon2id_impl_t *p = (argon2id_impl_t *)impl;
    do_hash(p, p->buf, p->buf_len, out, out_len);
    secure_zero(p->buf, p->buf_len);
    p->buf_len = 0;
}

static void argon2id_destroy_fn(void *impl)
{
    argon2id_impl_t *p = (argon2id_impl_t *)impl;
...
hash_adapter_t *argon2id_adapter_create(void)
{
    ...
    argon2id_impl_t *p = (argon2id_impl_t *)malloc(sizeof(argon2id_impl_t));
    ...
    a->digest_size = 32;
    a->block_size  = 64;
    a->hash_fn     = argon2id_hash_fn;
    a->init_fn     = argon2id_init_fn;
    a->update_fn   = argon2id_update_fn;
    a->final_fn    = argon2id_final_fn;
    a->destroy_fn  = argon2id_destroy_fn;
    return a;
}
...
{
    if (!a || !a->impl) return;
    argon2id_impl_t *p = (argon2id_impl_t *)a->impl;
